...
> {
  try {
    ...
    const { data, error } = await sb
      ...
      .select("partner_id, policy_id, policy_version, sui_address, status, expires_at")
...
function bindVerificationToContinuation(input: {
  ...
  if (!subject) return { ok: false, code: "invalid_session" };
  try {
  ...
  if (
    typeof input.row.policy_version === "number"
    && input.stored.policyVersion != null
    && input.row.policy_version !== input.stored.policyVersion
  ) {
...
export async function resolveBoundPartnerContinuation(input: {
  ...
  return { ok: true, stored, bindingPresent };
}
