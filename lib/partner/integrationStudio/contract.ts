...
export const INTEGRATION_STUDIO_SOLANA_ONCHAIN_NOTICE =
  "Private holder verification, then the server verifies the current receipt. Abraxas signs a narrow Solana authorization. The partner transaction includes Ed25519 verification immediately before the gate. The reference gate binary is deployed on devnet, but GateConfig and partner registration are not complete; no authorization can be used yet. No Mainnet deployment.";
...
export const INTEGRATION_STUDIO_CROSS_CHAIN_PROTOCOL_NOTICE =
  "Build a cross-chain protocol gate. Private proof, fresh consent, audience-bound receipt, server re-check, one-time chain authorization, then partner-owned activate_protocol_access. Access is short-lived until attestation expiry. The Solana reference gate binary is on devnet but remains unconfigured and unregistered; there is no live end-to-end chain action, Mainnet, Arc, USDC, or Utila integration.";
export const INTEGRATION_STUDIO_TESTNET_GATE_NOTICE =
  "Human-operated CLI to plan, validate-plan, confirm a non-broadcasting deploy handoff, verify, and register a partner-owned gate on Solana devnet or approved EVM Sepolia. A reusable institutional gate requires V2 attestations. Organization, actor, category, subject, and expiry are attestation-only. Sequence: Institutional policy review required → Create V2 testnet deployment plan → Human deploys gate with a local toolchain → Verify exact deployed configuration → Register verified sandbox deployment → Request a fresh institutional presentation and chain attestation. No browser deploy button. Not Mainnet, Arc, USDC, Circle, Utila, or live KYB.";
