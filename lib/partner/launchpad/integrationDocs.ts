...
window.location.assign(verifyUrl.toString());`,
  ...
  "${SITE_URL}/api/v1/receipts/RECEIPT_ID"`,
    ...
    error_codes: [
      ...
      { code: "wrong_policy_version", description: "Receipt policy version does not match the pinned expected version" },
      { code: "policy_version_draft", description: "Draft policy versions cannot issue or authorize production receipts" },
      { code: "policy_version_deprecated", description: "This policy version is past its deprecation effective date" },
    ],
    sandbox_testing: [
    ...
    production_checklist: [
      ...
      "Request Production review after server-side readiness checks pass. A reviewer decides Production access.",
