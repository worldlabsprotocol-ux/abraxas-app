...
describe("productLoopStepThemes", () => {
  it("uses diagram mode for signin and consent", () => {
    expect(themeForStep("signin").mode).toBe("diagram");
    expect(themeForStep("consent").mode).toBe("diagram");
    expect(themeForStep("browse").mode).toBe("photo");
    expect(themeForStep("book").mode).toBe("photo");
  });
