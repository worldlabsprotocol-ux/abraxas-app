...
const COOLDOWN_MS = 3_000; // 3s between events per vault
function checkCooldown(vaultId: string): boolean {
  const last = cooldowns.get(vaultId) ?? 0;
  if (Date.now() - last < COOLDOWN_MS) return false;
  cooldowns.set(vaultId, Date.now());
  return true;
}
...
export async function triggerRiskEvent(vaultId: string): Promise<void> {
  ...
  if (!vault) return;
  // Rate limit: 3s cooldown per vault
  if (!checkCooldown(vaultId)) return;
