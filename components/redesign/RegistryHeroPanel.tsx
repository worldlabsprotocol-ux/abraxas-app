"use client";
// FILE: components/redesign/RegistryHeroPanel.tsx
// Infrastructure hero — diverse registry mosaic, not a single-asset billboard.

import { motion } from "framer-motion";
import { EXPLORE_ASSETS } from "@/lib/data/exploreAssets";
import { Btn } from "./ui";

const FONT = "'Inter',system-ui,-apple-system,sans-serif";
const MONO = "'JetBrains Mono','SF Mono',ui-monospace,monospace";
const ACCENT = "#10B981";

const MOSAIC = EXPLORE_ASSETS.filter(a => a.image).slice(0, 4);

export function RegistryHeroPanel() {
  return (
    <motion.div
      initial={{ opacity: 0, y: 24 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1], delay: 0.15 }}
      style={{
        position: "relative", borderRadius: 20, overflow: "hidden",
        border: "1px solid var(--border-strong)",
        boxShadow: "var(--shadow-glow)", background: "var(--surface-raised)",
      }}
    >
      <div style={{
        display: "grid",
        gridTemplateColumns: "repeat(2, 1fr)",
        gap: 3,
        background: "var(--bg)",
        aspectRatio: "4/3",
        minHeight: 320,
      }}>
        {MOSAIC.map((asset, i) => (
          <div key={asset.id} style={{ position: "relative", overflow: "hidden", background: "#06090B" }}>
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img
              src={asset.image}
              alt={asset.name}
              style={{
                width: "100%", height: "100%", objectFit: "cover", display: "block",
                objectPosition: asset.id === "genesis-asset" ? "78% center" : "center",
              }}
            />
            <div style={{
              position: "absolute", inset: 0,
              background: i === 0 ? "linear-gradient(135deg, rgba(6,9,11,0.15) 0%, transparent 60%)" : "rgba(6,9,11,0.25)",
            }} />
            {i === 0 && (
              <span style={{
                position: "absolute", bottom: 8, left: 8, right: 8,
                fontFamily: FONT, fontSize: "0.62rem", fontWeight: 700, color: "#fff",
              }}>
                {asset.name}
              </span>
            )}
          </div>
        ))}
      </div>

      <div style={{
        position: "absolute", inset: 0, pointerEvents: "none",
        background: "linear-gradient(to top, rgba(6,9,11,0.94) 0%, rgba(6,9,11,0.35) 45%, transparent 100%)",
      }} />

      <div style={{ position: "absolute", top: 16, left: 16, right: 16, display: "flex", justifyContent: "space-between", gap: "0.5rem", flexWrap: "wrap" }}>
        <span style={{
          fontFamily: MONO, fontSize: "0.55rem", fontWeight: 700,
          padding: "0.25rem 0.55rem", borderRadius: 999,
          background: "rgba(0,0,0,0.65)", color: ACCENT, border: "1px solid rgba(16,185,129,0.35)",
        }}>
          PUBLIC REGISTRY
        </span>
        <span style={{
          fontFamily: MONO, fontSize: "0.55rem", fontWeight: 700,
          padding: "0.25rem 0.55rem", borderRadius: 999,
          background: "rgba(0,0,0,0.55)", color: "rgba(255,255,255,0.85)",
          border: "1px solid rgba(255,255,255,0.15)",
        }}>
          {EXPLORE_ASSETS.length} assets · design partner phase
        </span>
      </div>

      <div style={{ position: "absolute", bottom: 16, left: 16, right: 16, pointerEvents: "auto" }}>
        <div style={{ fontFamily: FONT, fontSize: "1.05rem", fontWeight: 800, color: "#fff", marginBottom: "0.35rem" }}>
          Verified assets across classes
        </div>
        <div style={{ fontFamily: FONT, fontSize: "0.72rem", color: "rgba(255,255,255,0.78)", marginBottom: "0.75rem", lineHeight: 1.55 }}>
          Browse without ID checks. Multiple asset classes with transparent assurance levels.
        </div>
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          <Btn href="#registry" size="sm">Browse registry →</Btn>
          <Btn href="/passport" variant="tertiary" size="sm">Create passport</Btn>
        </div>
      </div>
    </motion.div>
  );
}
