...
export function VaultCard({ vault }: { vault: Vault }) {
  ...
  return (
    <div
      ...
      className="hover:border-gold/30 group"
      onMouseEnter={(e) => {
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "1.5rem" }}>
        <div>
          ...
          <div style={{ fontSize: "0.72rem", color: "var(--subtle)", letterSpacing: "0.05em" }}>
            {vault.assetClass}
          </div>
        </div>
        <div style={{
          ...
          border: `1px solid ${statusColors[vault.status] ?? "var(--line)"}20`,
          padding: "0.2rem 0.5rem",
          borderRadius: "4px",
        }}>
          <span style={{ width: "4px", height: "4px", borderRadius: "50%", background: statusColors[vault.status], flexShrink: 0, ...(vault.status === "operating" ? { animation: "pulse 2s ease-in-out infinite" } : {}) }} />
          {vault.status}
        </div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", marginBottom: "1.5rem" }}>
        {[
          { k: "TVL", v: formatCurrency(vault.tvl) },
          { k: "Yield YTD", v: `${vault.yieldYTD}%`, accent: "var(--green)" },
          ...
          <div key={k} style={{ display: "flex", justifyContent: "space-between", fontSize: "0.8rem" }}>
            ...
            <span style={{ color: accent ?? "var(--text)", fontFamily: mono ? "'JetBrains Mono', monospace" : "inherit", fontSize: mono ? "0.75rem" : undefined, fontWeight: accent ? 600 : 500 }}>
              {v}
            </span>
          </div>
        ))}
      ...
      <div style={{ borderTop: "1px solid var(--line)", paddingTop: "1rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ fontSize: "0.7rem", color: "var(--subtle)" }}>
          Since {vault.inceptionDate}
        </span>
        <span style={{ fontSize: "0.7rem", color: "var(--gold)" }}>
          {vault.defenseEvents} defense events →
        </span>
      </div>
    </div>
  );
