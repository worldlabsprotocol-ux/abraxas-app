...
import {
  ...
  startDemoMode, stopDemoMode, UserVault, VaultEvent, SophiaStrategy,
...
const LEVEL_BG: Record<string, string> = {
  LOW: "rgba(61,214,140,0.08)", MEDIUM: "rgba(240,217,138,0.06)",
  HIGH: "rgba(200,169,110,0.08)", CRITICAL: "rgba(242,107,107,0.08)",
};
...
function VaultCard({ vault, onTrigger, onSwitch }: {
  vault: UserVault; onTrigger: (id: string) => void; onSwitch: (id: string, s: SophiaStrategy) => void;
}) {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: "0.625rem", marginBottom: "0.75rem", flexWrap: "wrap" }}>
        <span style={{ fontSize: "0.58rem", padding: "0.12rem 0.45rem", borderRadius: "4px", background: "rgba(255,255,255,0.04)", border: "1px solid var(--line)", color: "var(--subtle)", letterSpacing: "0.06em" }}>Simulation Mode</span>
      ...
      {!hasVault && (
        <div style={{ background: "rgba(200,169,110,0.06)", border: "1px solid rgba(200,169,110,0.25)", borderRadius: "14px", padding: "2rem 1.5rem", marginBottom: "1rem", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: "0.625rem", justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={() => createVault(newStrategy)} style={{ background: "var(--gold)", color: "var(--void)", border: "none", borderRadius: "8px", padding: "0.75rem 1.75rem", fontWeight: 700, fontSize: "0.9rem", cursor: "pointer", fontFamily: "'Space Grotesk',sans-serif" }}>
              Create Vault
            </button>
            <button onClick={startDemoMode} style={{ background: "var(--surface)", color: "var(--text)", border: "1px solid var(--line)", borderRadius: "8px", padding: "0.75rem 1.5rem", fontWeight: 600, fontSize: "0.85rem", cursor: "pointer" }}>
              Enter Demo Mode
            </button>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(100%,300px),1fr))", gap: "1rem" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            {engine.vaults.slice(0, 3).map((v) => (
              <VaultCard key={v.id} vault={v} onTrigger={triggerRiskEvent} onSwitch={switchStrategy} />
            ))}
            <div style={{ display: "flex", gap: "0.5rem" }}>
              <button onClick={() => createVault(newStrategy)} style={{ flex: 1, background: "var(--surface)", color: "var(--text)", border: "1px solid var(--line)", borderRadius: "7px", padding: "0.55rem", fontSize: "0.72rem", cursor: "pointer", fontWeight: 600 }}>+ New Vault</button>
              <button onClick={engine.demoRunning ? stopDemoMode : startDemoMode} style={{ flex: 1, background: engine.demoRunning ? "rgba(242,107,107,0.1)" : "rgba(200,169,110,0.08)", color: engine.demoRunning ? "#f26b6b" : "var(--gold)", border: `1px solid ${engine.demoRunning ? "rgba(242,107,107,0.3)" : "rgba(200,169,110,0.25)"}`, borderRadius: "7px", padding: "0.55rem", fontSize: "0.72rem", cursor: "pointer", fontWeight: 600 }}>
