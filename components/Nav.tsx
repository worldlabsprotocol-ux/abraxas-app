...
const MENU_GROUPS = [
  ...
  },
  {
    ...
    links: [
      ...
      { href: "/transparency", label: "Defense Log"   },
    ...
    <>
      <nav style={{
        ...
        <div style={{ display: "none", alignItems: "center", gap: "1.5rem", flex: 1 }} className="md:flex">
          {PRIMARY.map((link) => (
            <Link key={link.href} href={link.href} style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "0.68rem", letterSpacing: "0.08em", textTransform: "uppercase", textDecoration: "none", color: pathname === link.href ? "var(--gold)" : "var(--muted)", position: "relative", transition: "color 0.2s" }}>
              {pathname === link.href && <span style={{ position: "absolute", bottom: "-4px", left: 0, right: 0, height: "1px", background: "var(--gold)" }} />}
              {link.label}
            </Link>
          ))}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "0.4rem", flexShrink: 0 }}>
          <div style={{ display: "none" }} className="md:flex"><LanguageSelector /></div>
          ...
          <button onClick={() => setOpen((v) => !v)} aria-label="Menu" style={{ background: "none", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "5px", width: "28px", height: "28px", minWidth: "28px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "4px", cursor: "pointer", flexShrink: 0, padding: 0 }}>
            {open
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 49, background: "rgba(2,3,10,0.98)", backdropFilter: "blur(24px)", paddingTop: "56px", overflowY: "auto" }}>
          <div style={{ padding: "1.25rem 1.25rem 2rem" }}>
            <Link href="/onboard" style={{ display: "block", textDecoration: "none", marginBottom: "1.5rem" }}>
              <div style={{ background: "var(--gold)", color: "var(--void)", borderRadius: "10px", padding: "0.875rem", textAlign: "center", fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: "0.875rem", textTransform: "uppercase", letterSpacing: "0.06em" }}>Get Started →</div>
            </Link>
            {MENU_GROUPS.map((group) => (
              <div key={group.label} style={{ marginBottom: "1.5rem" }}>
                <p style={{ fontSize: "0.56rem", letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--subtle)", marginBottom: "0.5rem", paddingBottom: "0.5rem", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>{group.label}</p>
                {group.links.map((link) => (
                  <Link key={link.href} href={link.href} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.7rem 0", fontFamily: "'Space Grotesk', sans-serif", fontSize: "0.95rem", fontWeight: pathname === link.href ? 700 : 400, textDecoration: "none", color: pathname === link.href ? "var(--gold)" : "var(--text)", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    {link.label}
            ...
            <div style={{ marginBottom: "1rem" }}><LanguageSelector /></div>
