...
// World Labs, the first tokenized business on Abraxas. The official
// logo is now the primary visual (00001.jpg, in /public/assets/
// worldwearables/, that's where the working file ended up). The
// hoodie product is its own separate, visually distinct mini-card
// below, deliberately not nested inside the main card, so it has its
// own presence instead of getting lost as a footnote under World Labs.
...
// Hoodie product photo, separate from the logo.
...
// Small helper: tries each candidate filename in order, shows nothing
// rather than a broken image if all fail.
...
export function WorldLabsFeature({ onBuyNow }: WorldLabsFeatureProps) {
  return (
    <div>
      {/* MAIN CARD: World Labs, the business */}
        ...
        <div style={{ display:"flex", flexWrap:"wrap" }}>
          ...
          <div style={{ flex:"2 1 320px", padding:"1.25rem 1.5rem" }}>
            ...
            <p style={{ fontFamily:S, fontSize:"0.8rem",
              ...
              exercise, it's the same standard every other business on
              the platform will be held to, proven on the founder's own
              company before anyone else's.
            </p>
      ...
      {/* SEPARATE CARD: World Wearables hoodie, its own visual weight,
          not nested under World Labs so it doesn't get lost */}
      <div style={{ borderRadius:12, overflow:"hidden",
