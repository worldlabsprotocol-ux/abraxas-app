...
// Operational bottom nav — every tab ends in a state change or system action.
"use client";
...
export function BottomNav() {
  ...
  const { vaults, systemState } = useSystemState();
  ...
  return (
    <nav style={{
      ...
      {TABS.map((tab) => {
        ...
        // Show badge on Vaults tab if any vault is at risk
        ...
        return (
          <Link key={tab.key} href={tab.href} style={{ flex: 1, textDecoration: "none" }}>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "0.25rem", height: "100%", padding: "0.5rem 0", background: active ? c.bg : "transparent", boxShadow: active ? `inset 0 2px 0 ${c.active}` : "none", transition: "all 0.2s", cursor: "pointer", position: "relative" }}>
              ...
              <div style={{ filter: active ? `drop-shadow(0 0 6px ${c.active})` : "none", transform: active ? "translateY(-1px)" : "translateY(0)", transition: "all 0.15s" }}>
