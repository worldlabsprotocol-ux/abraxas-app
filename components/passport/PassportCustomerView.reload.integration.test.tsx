...
// FILE: components/passport/PassportCustomerView.reload.integration.test.tsx
...
const SUI = "0x" + "2".repeat(64);

function renderView(
  setup: ReturnType<typeof computePassportSetupState>,
  walletBindingStatus: "missing" | "active",
...
describe("PassportCustomerView reload secured state", () => {
  beforeEach(() => {
  ...
  it("shows Account secured after repair refresh and simulated reload", async () => {
    const unbound = computePassportSetupState({
      walletDone: true,
      identityStatus: "not_started",
      credentialStatus: "not_issued",
      walletBindingL3: false,
    });
    const bound = computePassportSetupState({
      walletDone: true,
      identityStatus: "not_started",
      credentialStatus: "not_issued",
    ...
    const view = renderView(unbound, "missing", onWalletBound);
