...
const STEPS = [
  ...
  { id: "identity", label: "Identity", sub: "Veriff Precheck" },
...
export function VerifyStepRail({
  ...
  return (
    <div style={{ display: "flex", gap: "0.35rem", flexWrap: "wrap", marginBottom: "1.25rem" }}>
      {STEPS.map((step, i) => {
        const done = step.id === "wallet" ? walletDone : earned[step.id] === "earned";
        const current = active === step.id || (step.id === "wallet" && !walletDone);
        const inProgress = step.id !== "wallet" && earned[step.id] === "in_progress";
        ...
        return (
          <div key={step.id} style={{
            ...
            opacity: step.id === "wallet" || walletDone || step.id === "identity" ? 1 : 0.65,
          }}>
            <div style={{ fontFamily: MONO, fontSize: "0.5rem", color: col, fontWeight: 700, marginBottom: "0.2rem" }}>
              {i + 1}. {done ? "DONE" : inProgress ? "IN REVIEW" : current ? "NOW" : "NEXT"}
            </div>
