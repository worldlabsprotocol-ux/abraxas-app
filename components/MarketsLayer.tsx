...
export function MarketsLayer() {
  ...
  // Filter to only assets with a real imagePath — no broken placeholder cards
  const inventoryAssets: InvAsset[] = (
    (rawInventory as any).assets ?? []
  ).filter((a:InvAsset) => a.imagePath && String(a.imagePath).trim().length > 0);
