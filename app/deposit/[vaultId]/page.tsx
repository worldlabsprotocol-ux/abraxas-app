...
type DepositState = "input" | "preview" | "confirming" | "done";
  ...
  return (
    <div style={{ maxWidth: "600px", margin: "0 auto", padding: "2.5rem 1.5rem" }}>
      ...
      <div style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: "14px", padding: "1.25rem 1.5rem", marginBottom: "1.5rem" }}>
        ...
        <div style={{ marginTop: "1rem", paddingTop: "1rem", borderTop: "1px solid var(--line)", display: "flex", gap: "1.5rem", fontSize: "0.72rem" }}>
          <div>
            <span style={{ color: "var(--subtle)" }}>Agent: </span>
            <span style={{ color: "var(--gold)", fontFamily: "'JetBrains Mono', monospace" }}>{agent?.name}</span>
          </div>
          <div>
            <span style={{ color: "var(--subtle)" }}>Current TVL: </span>
            <span>{formatCurrency(vaultPosition?.tvl ?? vault.tvl)}</span>
          </div>
          <div>
            <span style={{ color: "var(--subtle)" }}>Defense: </span>
          ...
          <div style={{ display: "flex", gap: "0.5rem", marginBottom: "1.5rem", flexWrap: "wrap" }}>
            {["500", "1000", "5000", "10000"].map((v) => (
              <button
                ...
                onClick={() => setAmount(v)}
                style={{
                  background: amount === v ? "rgba(200,169,110,0.1)" : "var(--surface)",
          ...
          {parsed > 0 && (
            <div style={{ background: "rgba(61,214,140,0.05)", border: "1px solid rgba(61,214,140,0.2)", borderRadius: "12px", padding: "1.25rem", marginBottom: "1.5rem" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem" }}>
                <div>
                  <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: "1.25rem", color: "var(--green)" }}>
              ...
              <p style={{ fontSize: "0.68rem", color: "var(--subtle)", marginTop: "0.75rem" }}>
                Based on {yieldRate}% APY · Projections are estimates, not guarantees.
              </p>
            </div>
          )}
        ...
        <>
          <div style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: "14px", padding: "1.5rem", marginBottom: "1.25rem" }}>
            ...
            {[
              ...
              { label: "Agent",           value: agent?.name ?? `AGENT-00${vault.agentId}`, mono: true },
              { label: "Wallet",          value: walletAddress ?? "Connected", mono: true },
              ...
              <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "0.6rem 0", borderBottom: "1px solid var(--line)" }}>
                ...
                <span style={{ fontSize: "0.78rem", fontWeight: bold ? 700 : 500, color: color ?? "var(--text)", fontFamily: mono ? "'JetBrains Mono', monospace" : "inherit" }}>
                  {value}
                </span>
              </div>
            ))}
          ...
          <div style={{ display: "flex", alignItems: "flex-start", gap: "0.75rem", marginBottom: "1.5rem", padding: "1rem", background: "rgba(107,140,255,0.04)", border: "1px solid rgba(107,140,255,0.12)", borderRadius: "10px" }}>
            ...
            <label htmlFor="agree" style={{ fontSize: "0.72rem", color: "var(--muted)", lineHeight: 1.65, cursor: "pointer" }}>
              I understand this is a beta product. Abraxas is non-custodial — I retain ownership of underlying assets. Projected yields are estimates. Agent activity is simulated in this beta version; real execution will be enabled at graduation.
            </label>
          </div>
      ...
      {step === "confirming" && (
        <div style={{ textAlign: "center", padding: "3rem 1.5rem" }}>
          ...
          <p style={{ fontWeight: 600, marginBottom: "0.5rem" }}>Preparing position…</p>
          <p style={{ fontSize: "0.78rem", color: "var(--muted)" }}>Assigning agent and minting Token-2022 position.</p>
        </div>
      )}
      ...
      {step === "done" && (
        <div style={{ textAlign: "center", padding: "3rem 1.5rem" }}>
          ...
          <p style={{ fontSize: "0.875rem", color: "var(--muted)", lineHeight: 1.7, marginBottom: "1.25rem" }}>
            {agent?.name} is now monitoring your position. Circuit defense is active.
          </p>
          ...
          <p style={{ fontSize: "0.78rem", color: "var(--muted)", marginBottom: "1.5rem", lineHeight: 1.6 }}>
            You are now an Abraxas operator.<br />View your operator ID and share your status.
          </p>
