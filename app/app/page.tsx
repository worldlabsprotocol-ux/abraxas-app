...
export default function DashboardPage() {
  ...
  const totalDeposited = active.reduce((s, p) => s + p.amount, 0);
  const annualYield    = active.reduce((s, p) => s + Math.round(p.amount * p.apy / 100), 0);
    ...
    <div style={{ maxWidth: "900px", margin: "0 auto", padding: "2.5rem 1.25rem 4rem" }}>
      ...
      {!walletConnected && (
        <div style={{ background: "rgba(200,169,110,0.06)", border: "1px solid rgba(200,169,110,0.2)", borderRadius: "12px", padding: "1.5rem", marginBottom: "1.5rem", textAlign: "center" }}>
          ...
          <p style={{ fontSize: "0.78rem", color: "var(--muted)", marginBottom: "1rem" }}>
            See your SOL balance, your positions, and your minted tokens.
          </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: "0.625rem", marginBottom: "1.5rem" }}>
          {[
            ...
            <div key={s.k} style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: "10px", padding: "0.875rem 1rem" }}>
              ...
              <div style={{
                fontWeight: 700, fontSize: s.mono ? "0.78rem" : "1rem",
      ...
      <div style={{ marginBottom: "2rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.875rem" }}>
          ...
          {active.length > 0 && (
            <Link href="/use" style={{ fontSize: "0.72rem", color: "var(--gold)", textDecoration: "none" }}>
              Withdraw →
            </Link>
          )}
        </div>
        ...
        {active.length === 0 ? (
          <div style={{ background: "var(--surface)", border: "1px dashed var(--line)", borderRadius: "12px", padding: "2.5rem 1.25rem", textAlign: "center" }}>
            <p style={{ fontSize: "0.85rem", color: "var(--muted)", marginBottom: "1rem" }}>
              {walletConnected ? "No active positions yet." : "Connect your wallet to see positions."}
            </p>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.625rem" }}>
            {active.map((p) => {
              ...
              const annual   = Math.round(p.amount * p.apy / 100);
              const explorer = `https://solscan.io/tx/${p.txSig}`;
              return (
                <div key={p.id} style={{ background: "var(--surface)", border: "1px solid var(--line)", borderRadius: "12px", padding: "1.1rem 1.25rem" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(110px, 1fr))", gap: "0.625rem", marginBottom: "0.75rem", paddingTop: "0.75rem", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                    <div>
                      ...
                      <div style={{ fontWeight: 600, fontSize: "0.85rem" }}>{fmtUSD(p.amount)}</div>
                    </div>
                    <div>
                    ...
                    <div>
                      <div style={{ fontSize: "0.58rem", color: "var(--subtle)", textTransform: "uppercase", letterSpacing: "0.06em" }}>Token</div>
                      <div style={{ fontWeight: 600, fontSize: "0.78rem", fontFamily: "'JetBrains Mono', monospace", color: "var(--gold)" }}>ABRAP</div>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: "0.625rem", flexWrap: "wrap" }}>
                    <a href={explorer} target="_blank" rel="noopener noreferrer" style={{ fontSize: "0.7rem", color: "var(--gold)", textDecoration: "none" }}>
                      View tx ↗
                    </a>
                    ...
                    <button onClick={() => router.push(`/use?id=${p.id}`)}
                      style={{ marginLeft: "auto", background: "transparent", color: "var(--text)", border: "1px solid var(--line)", borderRadius: "6px", padding: "0.3rem 0.7rem", fontSize: "0.7rem", fontWeight: 600, cursor: "pointer" }}>
                      Withdraw
                    </button>
                  </div>
                </div>
