...
export default function PartnerFlowDocsPage() {
  ...
  return (
    <RedesignPage maxWidth={920}>
      ...
      <ContentCard title="Choose your integration path">
      ...
      <ContentCard title="Partner Flow entry URL">
        <p style={body}>
      ...
      <ContentCard title="Lifecycle: evaluate → Passport → complete / refresh">
        <div style={{ display: "grid", gap: "0.75rem" }}>
      ...
      <ContentCard title="Callback query parameters (frozen — no PII)">
        <p style={body}>{PARTNER_FLOW_CALLBACK_PII_NOTE}</p>
        <BulletList items={[...PARTNER_FLOW_CALLBACK_PARAMS]} />
      </ContentCard>

      <ContentCard title="Receipt verification (server-side)">
        <p style={body}>
      ...
      <ContentCard title="Browser session vs server API key">
      ...
      <ContentCard title="Errors and status behavior">
      ...
      <ContentCard title="Copy-paste redirect">
        <pre style={{
          fontFamily: MONO, fontSize: "0.68rem", lineHeight: 1.55,
          padding: "0.85rem", borderRadius: 10, overflow: "auto",
          background: "var(--surface-inset)", border: "1px solid var(--border)",
      ...
      <ContentCard title="Operator provisioning">
        <p style={body}>
          Partner rows, policies, and callback allowlists are provisioned by Abraxas operators.
          There is no self-serve portal. See the onboarding checklist for exact requirements.
        </p>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginTop: "0.75rem" }}>
          <Link href="/design-partner">
            <Btn size="sm">Apply as design partner</Btn>
          </Link>
