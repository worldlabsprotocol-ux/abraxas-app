...
The public DEMO environment (`https://demo.abraxasworld.xyz`) is the same product with isolated test data. Circle there is testnet-only, receipt-gated, review-first, and requires explicit confirmation. It never moves funds automatically. See `docs/demo/DEMO_ENVIRONMENT.md`.

Also apply migration `089_circle_arc_testnet_settlement.sql` and then `090_circle_settlement_selection_replay.sql` to DEMO Supabase `ocntwbxarpjeixdnzide` before the first real transfer. Do **not** apply 089 or 090 to Production. Production Partner Flow OAuth resume needs `091_partner_flow_continuations.sql` only. Circle stays blocked when `VERCEL_ENV=production`.
