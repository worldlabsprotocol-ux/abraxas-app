...
The public DEMO environment is the same product with isolated test data. Submit remains receipt-gated and requires explicit `confirm_testnet_transfer`. Production does not apply migrations 089 or 090; Circle stays blocked when `VERCEL_ENV=production`. See `docs/demo/DEMO_ENVIRONMENT.md`.
