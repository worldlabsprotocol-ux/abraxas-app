#!/usr/bin/env npx tsx
// Human-operated devnet transaction only. Forbidden in CI, Vercel, tests and API runtimes.
...
async function main(): Promise<void> {
  ...
  if (args.length !== 3 || !args[0] || args[0].startsWith("-")
    ...
    process.stderr.write("usage: npx tsx scripts/solana-protocol-access-initialize-local.ts /path/to/public-signer-document.json --ownership-reviewed --confirm\n");
    process.exit(2);
  }
  ...
  const result = await initializeLocalSolanaProtocolAccess({
