...
describe("demoMigrationRunner manifest scope", () => {
  it("loads exactly the approved required-file order", () => {
    const plan = loadRequiredMigrationPlan();
    expect(plan).toHaveLength(DEMO_REQUIRED_MIGRATION_ORDER.length);
  ...
  it("dry-run is fully offline and performs no database work", () => {
    ...
    expect(output).toContain(`Approved manifest (${DEMO_REQUIRED_MIGRATION_ORDER.length} files)`);
